...
# Here, we import the two other .py files/modules:
# movie and fresh_tomatoes. Movie is where we have created
# the appropriate classes. We can create a new instance of
# a class by defining a variable as movie.Movie.
# Movie with the upper-case is the class, and movie with
# the lower case is the module we are importing from.
# That's the movie.py file. We are giving each movie the necessary
# attributes as defined in our movie file (title, storyline,
# image URL, youtube URL, release date). At the end,
# we are using the fresh_tomatoes module, which has a
# function entitled open_movies_page that takes the
# argument (movies). Movies, being our list of movies.
...
cyborg = movie.Movie("Cyborg",
                     "A post apocolyptic wasteland. Our only hope?"
...
knock_off = movie.Movie("Knock Off",
                        "Van Damme and Rob Schneider. This movie is not a"
                        " knock off, but it's about fake foregin goods.",
